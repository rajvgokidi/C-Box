...
int userns(struct child_config *config) {
  ...
  if(has_userns) {
    ...
    fprintf(stderr, "unsupported? continuining\n");
  }
...
int choose_hostname(char *buff, size_t len) {
  static const char* suits[] = {"mike", "harvey", "louis", "rachel"};
  static const char* major[] = {
    "ace", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "jack", "queen", "king"
  };
  static const char* minor[] = {
    "specter", "ross", "pearson", "litt"
  };

  struct timespec now = {0};
  clock_gettime(CLOCK_MONOTONIC, &now); // for rand generator?
...
int main(int argc, char **argv) {
  struct child_config config = {0}; // inits all members as zero in struct
  int err = 0;
  int option = 0;
  int sockets[2] = {0};
  pid_t child_pid = 0;
  int last_optind = 0; 
  while((option = getopt(argc, argv, "c:m:u:"))) { // colon represents each flag needs an argument
    switch(option) {
      case 'c':
